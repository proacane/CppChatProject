...
AuthenFriend::AuthenFriend(QWidget* parent) : QDialog(parent), ui(new Ui::AuthenFriend), _label_point(2, 6) {
    ...
    connect(ui->more_lb, &ClickedOnceLabel::clicked, this, &AuthenFriend::slot_show_more_label);
    initTipLbs();
    // 链接输入标签回车事件
    connect(ui->lb_ed, &CustomizeEdit::returnPressed, this, &AuthenFriend::slot_label_enter);
    connect(ui->lb_ed, &CustomizeEdit::textChanged, this, &AuthenFriend::slot_label_text_change);
    connect(ui->lb_ed, &CustomizeEdit::editingFinished, this, &AuthenFriend::slot_label_edit_finished);
    connect(ui->tip_lb, &ClickedOnceLabel::clicked, this, &AuthenFriend::slot_add_friend_label_by_click_tip);
    ...
    // 连接确认和取消按钮的槽函数
    connect(ui->cancel_btn, &QPushButton::clicked, this, &AuthenFriend::slot_apply_cancel);
    connect(ui->sure_btn, &QPushButton::clicked, this, &AuthenFriend::slot_apply_sure);
}
...
void AuthenFriend::addLabel(QString name) {
    ...
    connect(tmplabel, &FriendLabel::sig_close, this, &AuthenFriend::slot_remove_friend_label);
