...
TcpMgr::TcpMgr() : _host(""), _port(0), _b_recv_pending(false), _message_id(0), _message_len(0) {
    ...
    QObject::connect(&_socket, &QTcpSocket::readyRead, [&]() {
        ...
        forever {
            ...
            qDebug().noquote() << "receive body msg is " << messageBody;
    ...
    // 处理连接断开
    QObject::connect(&_socket, &QTcpSocket::disconnected, [&]() { qDebug() << "Disconnected from server."; });
    // 连接发送信号用来发送数据
...
void TcpMgr::initHandlers() {
    _handlers.insert(ID_CHAT_LOGIN_RSP, [this](ReqId id, int len, QByteArray data) {
        ...
        qDebug().quote() << "data is " << jsonObj;

        auto uid = jsonObj["uid"].toInt();
        auto name = jsonObj["name"].toString();
        auto nick = jsonObj["nick"].toString();
        auto avatar = jsonObj["avatar"].toString();
    ...
    _handlers.insert(ID_SEARCH_USER_RSP, [this](ReqId id, int len, QByteArray data) {
        ...
        qDebug().quote() << "Search info is " << jsonObj;
        auto search_info = std::make_shared<SearchInfo>(jsonObj["uid"].toInt(), jsonObj["name"].toString(),
                                                        ...
                                                        jsonObj["gender"].toInt(), jsonObj["avatar"].toString());
    ...
    _handlers.insert(ID_NOTIFY_ADD_FRIEND_REQ, [this](ReqId id, int len, QByteArray data) {
        ...
        qDebug().quote()<<"Receive friend apply from: " <<jsonObj;
        int from_uid = jsonObj["applyuid"].toInt();
        QString name = jsonObj["name"].toString();
        QString desc = jsonObj["desc"].toString();
        QString icon = jsonObj["avatar"].toString();
        QString nick = jsonObj["nick"].toString();
        int sex = jsonObj["gender"].toInt();
    ...
    _handlers.insert(ID_NOTIFY_AUTH_FRIEND_REQ, [this](ReqId id, int len, QByteArray data) {
        ...
        qDebug().quote()<<"Receive Notify auth friend request,data is "<<jsonObj;
    ...
    _handlers.insert(ID_AUTH_FRIEND_RSP, [this](ReqId id, int len, QByteArray data) {
        ...
        qDebug().quote()<<"data is "<<jsonObj;
        auto name = jsonObj["back"].toString();
        auto nick = jsonObj["nick"].toString();
        auto icon = jsonObj["avatar"].toString();
    ...
    _handlers.insert(ID_TEXT_CHAT_MSG_RSP, [this](ReqId id, int len, QByteArray data) {
        ...
        qDebug().quote() << "Receive Text Chat Rsp Success, data is "<< jsonObj;
        // TODO ui设置送达等标记 ...
    });
    // 收到了新消息
    _handlers.insert(ID_NOTIFY_TEXT_CHAT_MSG_REQ, [this](ReqId id, int len, QByteArray data) {
        ...
        qDebug().quote() << "Receive Text Chat Notify Success, data is "<< jsonObj;
