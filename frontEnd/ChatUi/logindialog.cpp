...
void LoginDialog::initHead() {
    // 加载图片
    QPixmap originalPixmap(":/res/wechatimg.png");
    // 设置图片自动缩放
...
void LoginDialog::initHttpHandlers() {
    ...
    _handlers.insert(ReqId::ID_LOGIN_USER, [this](QJsonObject jsonObj) {
        ...
        }else if(ErrorCodes::UserPasswordError){
            showTip(tr("邮箱或密码输入错误"), false);
            enableBtn(true);
            return;
        }else if(ErrorCodes::RPCFailed){
...
bool LoginDialog::checkPwdValid() {
    ...
    bool match = password_regex.match(pwd).hasMatch();
    if (!match) {
        // 提示字符非法
...
void LoginDialog::on_login_btn_clicked() {
    ...
    json_obj["password"] = hashPassword(pwd);
