...
void ResetDialog::initHandlers() {
    ...
    _handlers.insert(ReqId::ID_RESET_PWD, [this](QJsonObject jsonObj) {
        ...
        } else if (error == ErrorCodes::UserEmailNotMatch) {
            showTip(tr("请检查用户名和邮箱是否正确"), false);

        } else if (error == ErrorCodes::VerifyExpired) {
            showTip(tr("验证码已失效"), false);

        } else if (error == ErrorCodes::PasswordSame) {
            showTip(tr("新密码不能与原密码相同"), false);

        } else {
            showTip(tr("未知错误，请联系作者"), false);
        }
    });
}
...
void ResetDialog::on_sure_btn_clicked() {
    ...
    json_obj["password"] = hashPassword(ui->pwd_edit->text());
