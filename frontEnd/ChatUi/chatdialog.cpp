...
ChatDialog::ChatDialog(QWidget* parent) :
    QDialog(parent), ui(new Ui::ChatDialog), _b_loading(false), _mode(ChatUIMode::ChatMode),
    ...
    // 连接联系人页面点击好友申请条目的信号
    connect(ui->con_user_list, &ContactUserList::sig_switch_apply_friend_page, this,
...
void ChatDialog::handleGlobalMousePress(QMouseEvent* event) {
    ...
    QPoint posInSearchListInt = posInSearchList.toPoint();
    // 判断点击位置是否在聊天列表的范围内
    if (!ui->search_list->rect().contains(posInSearchListInt)) {
        // 如果不在聊天列表内，清空输入框
        ui->search_edit->clear();
        ShowSearch(false);
    }
...
void ChatDialog::loadMoreConUser() {
    ...
    if (friend_list.empty() == false) {
        for (auto& friend_ele : friend_list) {
            ...
            chat_user_wid->SetInfo(friend_ele->_uid, friend_ele->_name, friend_ele->_avatar);
            QListWidgetItem* item = new QListWidgetItem;
            // qDebug()<<"chat_user_wid sizeHint is " << chat_user_wid->sizeHint();
            item->setSizeHint(chat_user_wid->sizeHint());
